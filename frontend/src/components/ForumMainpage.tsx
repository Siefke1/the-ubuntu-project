...
const ForumMainpage: React.FC = () => {
  ...
  const [selectedTab, setSelectedTab] = useState(0); // 0: Recent (default), 1: Hot, 2: Pinned
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (response.ok && data.success) {
        const transformedCategories: ForumCategory[] = data.categories.map((category: ParsedCategory) => ({
          id: category.id,
          name: category.name,
  ...
  const fetchPosts = useCallback(async (page: number = 1, append: boolean = false, searchParam: string = '') => {
    try {
      if (page === 1) {
        setIsLoading(true);
      } else {
      ...
      const response = await fetch(`http://localhost:3001/api/posts?page=${page}&limit=10&sort=${sortParam}&category=${encodeURIComponent(categoryParam)}${searchParam}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      if (response.ok && data.success) {
        ...
        const transformedPosts: ForumPost[] = data.posts.map((post: ParsedPost) => ({
          id: post.id,
          title: post.title,
          author: post.author.firstName || post.author.username,
          authorAvatar: (post.author.firstName || post.author.username).charAt(0).toUpperCase(),
      ...
      <Container maxWidth="xl" sx={{ paddingTop: 2, paddingBottom: 4 }}>
        ...
        >
          <Box sx={{ mb: 4 }}>
            ...
            >
              {t.forum?.subtitle || 'Connect, Learn, and Share with the Ubuntu Community'}
            </Typography>
            ...
            <Box sx={{ maxWidth: 600, mx: 'auto', mb: 3 }}>
              <TextField
                ...
                InputProps={{
                  ...
                  endAdornment: searchQuery && isSearching ? (
                          ...
                          <Box sx={{ flex: 1 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                              {getPostStatusIcon(post) && (
                                <IconButton size="small" sx={{ color: getPostStatusColor(post) }}>
                                  {getPostStatusIcon(post)}
                                </IconButton>
                              )}
                              <Typography
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Avatar sx={{ width: 24, height: 24, bgcolor: colors.background.medium }}>
                              ...
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Visibility sx={{ fontSize: 16, color: colors.secondary }} />
                                <Typography variant="body2" sx={{ color: colors.secondary }}>
                                  {post.views}
                                </Typography>
                              </Box>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <ThumbUp sx={{ fontSize: 16, color: colors.accent }} />
                                <Typography variant="body2" sx={{ color: colors.accent }}>
                                  {post.likes}
                                </Typography>
                              </Box>
                              <Typography variant="body2" sx={{ color: colors.secondary, ml: 'auto' }}>
                                {post.lastActivity}
                              </Typography>
                            </Box>
                          </Box>
                  ...
                  <CardContent sx={{ flex: 1, overflow: 'hidden', p: 0 }}>
                    {categoriesLoading ? (
                      <Box sx={{ p: 2, textAlign: 'center' }}>
                        <Typography sx={{ color: colors.secondary }}>Loading categories...</Typography>
                      </Box>
                    ) : (
                      <Box
                        sx={{
                      ...
                      >
                          ...
                          {(showAllCategories ? categories : categories.slice(0, 5)).map((category) => (
                            <ListItem
                              ...
                              sx={{
                                ...
                                backgroundColor: activeCategory === category.name ? colors.accent + '20' : 'transparent',
                                border: activeCategory === category.name ? `1px solid ${colors.accent}` : '1px solid transparent',
                                '&:hover': {
                                  backgroundColor: activeCategory === category.name ? colors.accent + '30' : colors.accent + '10',
                                },
                              }}
                            >
                              <ListItemAvatar>
                                <Avatar sx={{ bgcolor: category.color, color: colors.textColorLight }}>
                                  {category.icon}
                                </Avatar>
                              </ListItemAvatar>
                              <ListItemText
                        ...
                        {categories.length > 5 && !showAllCategories && (
                          <Box sx={{ p: 2, textAlign: 'center', borderTop: `1px solid ${colors.secondary}20` }}>
                            <Button
                              size="small"
                  ...
                  <CardContent>
                    <Stack spacing={2}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Typography variant="body2" sx={{ color: colors.accent, fontWeight: 'bold' }}>
                          1,234
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
